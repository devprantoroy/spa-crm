            ...
            <div class="card">
                 <form  @submit.prevent="getResults" >
                    <div class="card-body">
                        <div class="form-row">
                            <div class="col-md-6">
                                <label for="search_super_code" class="col-form-label-sm">スーパーコード</label>
                                <input type="text" class="form-control form-control-sm" id="search_super_code" v-model="formPaginate.search_super_code" placeholder="スーパーコードで検索">
                            </div>
                            <div class="col-md-6">
        ...
        <div class="col-md-4" >
            <div class="card">
                <div class="card-body">
                    ...
                    <label for="search_super_code" class="col-form-label-sm">Import Buyer</label>
                    <div class="custom-file ">
                        ...
                        <label class="custom-file-label col-form-label-sm" for="file_url"> <small>Import Buyer</small> </label>
            ...
            <div class="card">
                <div class="card-header bg-secondary">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <button type="button" @click="downloadBuyerCsv" class="btn btn-warning btn-sm float-right btn-block"> <b-icon icon="cloud-download" class="far" font-scale="1.2"></b-icon>Export</button>
                        </div>

                        <div class="col-md-2">
                            <button type="button" class="btn btn-info btn-sm float-right btn-block" data-toggle="modal" data-target="#exampleModal"><b-icon icon="plus" class="far" font-scale="1.2"></b-icon> New Buyer</button>
                        </div>
                    </div>
                </div>
                <div class="card-body table-responsive">
                    <table class="table  table-sm  mb-5  table-hover">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th class="pointer_class" @click="sorting('super_code')">S. Code <span class="float-right" :class="iconSet('super_code')"></span></th>
                        ...
                        <tbody>
                            <tr  v-for="(buyer,index) in buyerList" :key="buyer.buyer_id">
                                <th scope="row">{{pageNo + index}}</th>
                                <td>{{buyer.super_code}}</td>
                                <td>{{buyer.name}} [{{buyer.name_kana}}]</td>
                                <td>{{buyer.tel}}</td>
                                <td>{{buyer.fax}}</td>
                                <td>{{buyer.post_code}}</td>
                                <!-- <td>{{buyer.address}}</td> -->
                                <td>{{buyer.billing_tel}}</td>
                                <td>{{buyer.billing_fax}}</td>
                                <td>{{buyer.billing_post_code}}</td>
                    ...
                    <!-- <ul>
                        <li v-for="post in PaginateData.data" :key="post.buyer_id">{{ post.name }}</li>
                    </ul> -->
                    <!-- <advanced-laravel-vue-paginate :data="PaginateData" @paginateTo="getResults"/> -->
                </div>
            </div>
        </div>
...
import AdvancedLaravelVuePaginate from 'advanced-laravel-vue-paginate';
    export default {
        ...
        methods:{
            downloadBuyerCsv(){
                 axios.post('buyer-csv-download',this.formPaginate)
                    .then(({data}) => {
            ...
            toggleModal(id) {
                 this.axios
                    .delete(`buyer-manager/${id}`)
                    .then(response => {
                        if(response.data.success){
                            let i = this.buyerList.map(data => data.id).indexOf(id);
            ...
            getResults(page = 1) {
                ...
                axios.post('buyer-manager' ,this.formPaginate)
                    .then(response => {
                        console.log(response.data);
                        this.buyerList = response.data.data;
